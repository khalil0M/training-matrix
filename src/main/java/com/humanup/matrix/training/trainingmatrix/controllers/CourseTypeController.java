...
public class CourseTypeController {
    @Autowired
    private CourseTypeBS courseTypeBS;

    @Operation(summary = "Create new course type", description = "Create new course type", tags = { "course type" })
    @RequestMapping(value="/coursetype", method=RequestMethod.POST,consumes={ "application/json"})
    @ResponseBody
    public ResponseEntity createCourseType(@RequestBody final CourseTypeVO courseType) {
        final Optional<Object> courseTypeFound = Optional.ofNullable(courseTypeBS.getCourseTypeByTitle(courseType.getTypeTitle()));
    ...
    @Operation(summary = "Find course type by id", description = "Find course type by id.", tags = { "course type" })
    ...
    public ResponseEntity getCourseTypeById(@RequestParam(value="id") final long id) {
        final Optional<CourseTypeVO> courseTypeFound = Optional.ofNullable(courseTypeBS.getCourseTypeById(id));
    ...
    @Operation(summary = "Find all course types", description = "Find all course types", tags = { "course type" })
