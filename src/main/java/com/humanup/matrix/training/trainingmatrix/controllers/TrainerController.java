...
public class TrainerController {
    @Autowired
    private TrainerBS trainerBS;

    @Operation(summary = "Create new trainer", description = "Create new trainer", tags = { "trainer" })
    @RequestMapping(value="/trainer", method=RequestMethod.POST,consumes={ "application/json"})
    @ResponseBody
    public ResponseEntity createTrainer(@RequestBody final TrainerVO trainer) {
        final Optional<Object> trainerFound = Optional.ofNullable(trainerBS.getTrainerByEmail(trainer.getEmail()));
    ...
    @Operation(summary = "Find trainer by id", description = "Find trainer by id.", tags = { "trainer" })
    ...
    @Operation(summary = "Find trainer by email", description = "Find trainer by email.", tags = { "trainer" })
    @RequestMapping(value="/trainer/email", method=RequestMethod.GET)
    @ResponseBody
    public ResponseEntity getTrainerByEmail(@RequestParam(value="email", defaultValue="robot@sqli.com") final String email){
        final Optional<TrainerVO> trainerFound = Optional.ofNullable(trainerBS.getTrainerByEmail(email));
        if(trainerFound.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }
        return ResponseEntity.status(HttpStatus.OK).body(trainerFound.get());
    ...
    @Operation(summary = "Find trainer by phone", description = "Find trainer by phone.", tags = { "trainer" })
    ...
    public ResponseEntity getTrainerByPhone(@RequestParam(value="phone") final String phone){
        final Optional<TrainerVO> trainerFound = Optional.ofNullable(trainerBS.getTrainerByPhone(phone));
        if(trainerFound.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }
        return ResponseEntity.status(HttpStatus.OK).body(trainerFound.get());
    ...
    @Operation(summary = "Find all trainers", description = "Find all trainers", tags = { "trainer" })
    ...
    @Operation(summary = "Find all trainers by name", description = "Find all trainers by name", tags = { "trainer" })
